    ...
    return (
      <Card bordered={false}>
        ...
        <PaymentList
          ...
          rowClassName={$record => {
            if (
              ($record.get(config.paymentFields.print) === 'n' &&
                $record.get('approStatus') === config.approStatus.toApproByCashier) ||
              $record.get('approStatus') === config.approStatus.payFailed
            ) {
